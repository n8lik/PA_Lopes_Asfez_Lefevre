...
function updateHousing($id, $title, $type_location, $ammountRoom, $experienceType, $guestCapacity, $propertyArea, $contactPhone, $time){
    $db = connectDB();
    $queryprepare = $db->prepare("UPDATE housing SET title = :title, type_location = :type_location, ammount_room = :ammount_room, management_type = :experienceType, guest_capacity = :guestCapacity, property_area = :propertyArea, contact_phone = :contactPhone, contact_time = :time, is_validated = :validated WHERE id = :id");
    $queryprepare->execute(['title' => $title, 'type_location' => $type_location, 'ammount_room' => $ammountRoom, 'experienceType' => $experienceType, 'guestCapacity' => $guestCapacity, 'propertyArea' => $propertyArea, 'contactPhone' => $contactPhone, 'time' => $time, 'id' => $id, 'validated' => 0]);
}
...
{
    $conn = connectDB();
    $req = $conn->prepare("SELECT * FROM performances WHERE user_id = :id");
    $req->execute(['id' => $id]);
    return $req->fetchAll();
